...
void ComputePBRDirectionalLight(float4 diffuse, float4 specular, float roughness, float metal ,PBRDirectionalLight L, 
						     ...
						     out float4 color)
{
	...
	//{
		color.xyz = PBRColor(normal, toEye, lightVec, diffuse.xyz, L.Diffuse.xyz, specular.xyz, roughness, metal);
				  ...
				  out float4 color)
{
	...
	//if( diffuseFactor > 0.0f )
	{
		PBRColor(normal, toEye, lightVec, diffuse.xyz, L.Diffuse.xyz, specular.xyz, roughness, metal);
	}
...
float4 PS(VertexOut pin, uniform int gLightCount, uniform bool gUseTexure, uniform bool gAlphaClip, uniform bool gFogEnabled) : SV_Target
{
	...
	float4 diffuse = gMaterial.diffuse;
    if(gUseTexure)
	{
		// Sample texture.
	...
	{  
		...
		{
			float4 color1;
			ComputePBRDirectionalLight(diffuse, gMaterial.specular, gMaterial.roughness, gMaterial.metal, gDirLights[i], pin.NormalW, toEye, 
				color1);

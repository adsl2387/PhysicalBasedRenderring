...
#pragma region CommonPassEffect
class CommonPassEffect:public Effect
{
public:
	CommonPassEffect(ID3D11Device* device, const std::wstring& filename);
	~CommonPassEffect();
	...
	void SetFogStart(float f)                           { FogStart->SetFloat(f); }
	void SetFogRange(float f)                           { FogRange->SetFloat(f); }
	void SetDirLights(const PBRDirectionalLight* lights)   { DirLights->SetRawValue(lights, 0, 3*sizeof(PBRDirectionalLight)); }
...
public:
	...
	static CommonPassEffect* CommonPassFX;
